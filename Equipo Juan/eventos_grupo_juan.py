...
root = tk.Tk()
root.title("Bomberos Voluntarios Totoras")
root.attributes("-fullscreen", True)
root.config(background="#333")

ttk.Label(root, text="Seleccione lo que desee hacer", font=("arial", 25), foreground="#fff", background="#333").place(x=500, y=200)

salir = tk.Button(root, text="Cerrar programa", background="#FFA82B", command=cerrarPrograma)
salir.place(x=675, y=600)
...
def menuEventos():
    ...
    def formulario_evento():
        ...
        tk.Button(form, text="Guardar", command=guardar, background="#FFA82B", font=('arial', 10)).place(pady=15)   
    def modificarEventos():
        ...
        def elegirEvento():
            ...
            def actualizarDatos():
                ...
                if idE.get() == "" or nombreE.get() == "" or puntos.get() == "":
    ...
    tk.Button(eventos, text="Menú principal", background="#FFA82B", width=20, height=2, command=eventos.destroy).place(x=700, y=550)
    

tk.Button(root, text="Eventos", background="#FFA82B", width=20, height=2, command=menuEventos).place(x=450, y=400)
tk.Button(root, text="Personal", background="#FFA82B", width=20, height=2, command=personal_grupo_juan.menuPersonal).place(x=850, y=400)
